...
tf.app.flags.DEFINE_float('drop_ratio', 0.5, "ドロップ率")
...
tf.app.flags.DEFINE_string('save_data_path', './../../../data/classification/cifar10/dropout/0704/', "データ保存先")
...
def deepnn(x):
    ...
    h_6 = tf.nn.relu(tf.matmul(h_5_out, W_6) + b_6)
    h_6_out = tf.nn.dropout(h_6, keep_prob)

    # 7 layer
    W_7 = weight_variable([32*32, 32*32])
    b_7 = bias_variable([32*32])
    h_7 = tf.nn.relu(tf.matmul(h_6_out, W_7) + b_7)
    h_7_out = tf.nn.dropout(h_7, keep_prob)

    # output
    W_out = weight_variable([32*32, 10])
    b_out = bias_variable([10])

    y_out = tf.matmul(h_7_out, W_out) + b_out
...
def main(argv):
    ...
    grad = tf.train.GradientDescentOptimizer(FLAGS.learning_rate).compute_gradients(cross_entropy)[4]
    ...
    save_path = FLAGS.save_data_path + 'drops_{}_batch_{}_alpha_{}/'.format(
        FLAGS.drop_ratio, FLAGS.batch_size, FLAGS.learning_rate)
    ...
    for _iter in range(FLAGS.iteration):
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(FLAGS.step):
                ...
                _, cross_entropy_curr, train_accuracy_batch, A = sess.run(
                    [train_step, cross_entropy, accuracy, grad],
                    feed_dict={x: _train_images_batch, y_: _trains_labels_batch, keep_prob: FLAGS.drop_ratio})
                ...
                if i % 5000 == 0 or i == FLAGS.step - 1:
                    ...
                    while _step < num_iter:
                        ...
                        train_accuracy = accuracy.eval(feed_dict={
                            x: _train_images, y_: _trains_labels, keep_prob: 1.0})
                    ...
                    while _step < num_iter:
                        ...
                        test_accuracy = accuracy.eval(feed_dict={
                            x: _test_images, y_: _test_labels, keep_prob: 1.0})
