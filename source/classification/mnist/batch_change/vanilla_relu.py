...
def deepnn(x, phase_train):
    ...
    if phase_train == True:
        ...
        h_j_1 = tf.scatter_update(h_j_1, h_1_index, tf.zeros_like(h_1_index, tf.float32))
    ...
    if FLAGS.layer_size == 1:
        ...
        entropy_all, s_index = compute_entropy_with_svd(jacobian)
        return y_out, entropy_all, h_1, s_index
    ...
    if phase_train == True:
        ...
        h_j_2 = tf.scatter_update(h_j_2, h_2_index, tf.zeros_like(h_2_index, tf.float32))
    ...
    if FLAGS.layer_size == 2:
        ...
        entropy_all, s_index = compute_entropy_with_svd(jacobian)
        return y_out, entropy_all, h_2, s_index
    ...
    if phase_train == True:
        ...
        h_j_3 = tf.scatter_update(h_j_3, h_3_index, tf.zeros_like(h_3_index, tf.float32))
    ...
    if FLAGS.layer_size == 3:
        ...
        entropy_all, s_index = compute_entropy_with_svd(jacobian)
        return y_out, entropy_all, h_3, s_index
    ...
    if phase_train == True:
        ...
        h_j_4 = tf.scatter_update(h_j_4, h_4_index, tf.zeros_like(h_4_index, tf.float32))
    ...
    if FLAGS.layer_size == 4:
        ...
        entropy_all, s_index = compute_entropy_with_svd(jacobian)
        return y_out, entropy_all, h_4, s_index
    ...
    if phase_train == True:
        ...
        h_j_5 = tf.scatter_update(h_j_5, h_5_index, tf.zeros_like(h_5_index, tf.float32))
    ...
    if FLAGS.layer_size == 5:
        ...
        entropy_all, s_index = compute_entropy_with_svd(jacobian)
        return y_out, entropy_all, h_5, s_index
    ...
    if phase_train == True:
        ...
        h_j_6 = tf.scatter_update(h_j_6, h_6_index, tf.zeros_like(h_6_index, tf.float32))
    ...
    if FLAGS.layer_size == 6:
        ...
        entropy_all, s_index = compute_entropy_with_svd(jacobian)
        return y_out, entropy_all, h_6, s_index
    ...
    if phase_train == True:
        ...
        h_j_7 = tf.scatter_update(h_j_7, h_7_index, tf.zeros_like(h_7_index, tf.float32))
    ...
    entropy_all, s_index = compute_entropy_with_svd(jacobian)
    return y_out, entropy_all, h_7, s_index
...
def compute_entropy_with_svd(jacobian):
    ...
    #s_index = tf.shape(tf.where(s == np.nan)
    #_s = tf.scatter_update(s, s_index, tf.ones_like(s_index, tf.float32))
    #if self.layer_method == "each":
    #s_index = tf.shape(tf.where(tf.abs(s) <= 0.1 ** 15))[0]
    _s = tf.maximum(tf.abs(s), 0.1 ** 15)
    log_determine = tf.log(_s) #+ tf.cast(s_index, tf.float32) * tf.constant(15.0 * np.log(10.0), tf.float32, shape=[1])
...
def main(argv):
    ...
    # model
    y_out, entropy_all, h_last, s_index = deepnn(x, phase_train)
    ...
    for _iter in range(FLAGS.iteration):
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(FLAGS.step):
                ...
                _, cross_entropy_curr, train_accuracy, s_index_curr = sess.run([
                    train_step, cross_entropy, accuracy, s_index], feed_dict={x: batch[0], y_: batch[1], phase_train: True})
