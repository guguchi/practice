...
tf.app.flags.DEFINE_string('save_data_path', './../../../data/classification/mnist/vanilla_batch/', "データ保存先")
...
def plot(samples, layer_samples):
    ...
    for (i, sample) in enumerate(samples):
        ...
        ax.set_yticklabels([])
        ax.set_aspect('equal')
        plt.imshow(layer_samples.reshape(28, 28), cmap='Greys_r')
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(FLAGS.step):
                ...
                if _iter == 0 and (i % 5000 == 0 or i == FLAGS.step - 1):
                    ...
                    layer_samples = self.sess.run([h_5],
                                  feed_dict={x: samples})

                    fig = plot(samples[0], layer_samples[0])
